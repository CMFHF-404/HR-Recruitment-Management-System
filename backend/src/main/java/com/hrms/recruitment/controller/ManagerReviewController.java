...
public class ManagerReviewController {
    ...
    public ManagerReviewController(ResumeScreeningRepository screenings, RecruitmentService service) {
    ...
    public ApiResponse<ResumeScreening> update(@PathVariable Long candidateId,
        ...
        return ApiResponse.ok("主管确认状态已更新",
                service.updateManagerReview(candidateId, request.status(), request.comment()));
    }

    public record ManagerReviewRequest(@NotNull ManagerReviewStatus status, String comment) {}
}
